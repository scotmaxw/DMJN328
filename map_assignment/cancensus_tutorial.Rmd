...
library(knitr)
knitr::opts_chunk$set(echo = TRUE, results='markup', eval=T, message=F, tidy=T, warning=F, fig.width=3, fig.height=3, fig.align='center', include=T, out.width="50%")
```
...
As with everything in this course, it is very useful to set up a folder with an RStudio project folder specifically for your map assignment. I have already actually created one on the course GitHub page. If you are able to to pull changes from the DMJN328 course page in GitHub, by following the instructions [here](https://github.com/sjkiss/DMJN328/raw/master/Using_github/syncing_to_course_repository.pdf), then you should see a folder called "map assignment" in your course folder. 


```{r map, echo=F, include=T, out.width="50%"}

knitr::include_graphics("images/map_assignment.png")
...
```{r api2}
include_graphics("https://raw.githubusercontent.com/sjkiss/DMJN328/master/images/api.png")
```
...
When you have a geographic boundary area that you are interested in playing with, select it and it should turn *blue*. If you click the same area again, it should deselct the region.  this is what it looks like if you have selected Ontario.
...
go back to the web browser and click on "Variable Selection" Honestly, this product is so incredible it kind of makes your head spin. Bear with me.

```{r kitchener-variables}
...
kitchener5 <- get_census(dataset='CA16', regions=list(CSD="3530013"), vectors=c("v_CA16_9","v_CA16_8"), labels="detailed", geo_format="sf", level='DA')
```
